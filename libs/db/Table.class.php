 ...
 * @ IM表操作基类
 *   增、删、改、查、分页；
 * @method create ((array)$param);
 * @method remove ((array)$param);
 * @method update ((array)$param);
 * @method where((array)$where)->query();
 * @method page ((string)$sql, (array)$param, (int)$min_id, (int)$max_id, (int)$count);
 */
class Table extends DBConnManager {
    ...
    public static function getConn() {
        ...
        $singletons[$database] = new Table($database, $readRetry, $writeRetry, $connectRetry);
        return $singletons[$database];
    }

    function __construct($database, $readretry, $writeretry, $connectretry) {
    ...
    public function get_last_sql(){
        $data =  \Phplib\DB\SQLMonitor::getMonitor()->dump();
        return $data;
        //return $this->conn->getLastSql();
    }
     ...
     * 创建插入数据
     */
    public function insert ($param, $virual_data = false) {
        ...
        $fields = join(', ', $fields);
        $values = join(', ', $values);
        $sql  = "insert into /*passport*/ {$this->table} ($fields) value ($values)";
        $res = $this->write($sql, $param);
        if ($res  == 0) {
            return false;
        }

        $last_id =  (int)$this->getInsertId();
        if ($virual_data) {
     ...
     * replace
     */
    public function replace_into ($param, $master = false) {
        $keys = array_keys($param);
        ...
        $res = $this->write($sql, $param, $master);
        return $res;
    }
    ...
    public function update ($param = array()) {
        ...
        $res = $this->write($sql, $update_param);
     ...
     * 查询
     */
    public function query ($fields = '*', $master = false, $hash_key = null) {
        $where = $this->adapter($param);
        $order = $this->order();

        if (!$fields) $fields = $this->get_all_fields();
        $sql  = "select $fields from {$this->table} where 1 and $where $order";

        $res = $this->read($sql, $param, $master, $hash_key);
        return $res;
    }
