...
require_once(dirname(__FILE__) . '/mlog/lib/writer/Writer.class.php');
require_once(dirname(__FILE__) . '/mlog/lib/writer/Writer_Std.class.php');
...
class MLog
{
    ...
    **/
     ...
     * @return  public static function 
    **/
    public static function setLogApp($app) 
    {
        if (!is_string($app)) {
            return false;
        }
    ...
    **/
    public static function getLogPath() 
    {
        if (self::$strLogPath == null) {
            self::$strLogPath = MLogConfig::$log_path;
        }
    ...
    **/
    public static function getDataPath() 
    {
        if (self::$strDataPath == null) {
            self::$strDataPath = MLogConfig::$data_path;
        }
    ...
    public function parseFormat($format) {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            switch($code) {
            case 'h':
                $action[] = "(defined('CLIENT_IP')? CLIENT_IP : MLog::getClientIp())";
                break;
            case 't':
            ...
            case 'e':
                ...
                break;
            case 'f':
                $action[] = 'MLog::$current_instance->current_file';
                break;
            case 'H':
            ...
            case 'L':
                $action[] = 'MLog::$current_instance->current_log_level';
                break;
            case 'N':
                $action[] = 'MLog::$current_instance->current_line';
                break;
            case 'E':
                $action[] = 'MLog::$current_instance->current_err_no';
                break;
            case 'l':
                $action[] = "MLog::genLogID()";
                break;
            case 'u':
                ...
                $action[] = "((defined('CLIENT_IP') ? CLIENT_IP: MLog::getClientIp()) . ' ' . \$____user____['uid'] . ' ' . \$____user____['uname'])";
                break;
            case 'S':
                if ($param == '') {
                    $action[] = 'MLog::$current_instance->get_str_args()';
                } else {
                    $param_name = var_export($param, true);
                    if (!isset($prelim_done['S_'.$param_name])) {
                        $prelim[] = 
                            "if (isset(MLog::\$current_instance->current_args[$param_name])) {
                            \$____curargs____[$param_name] = MLog::\$current_instance->current_args[$param_name];
                            unset(MLog::\$current_instance->current_args[$param_name]);
                ...
                break;
            case 'M':
                $action[] = 'MLog::$current_instance->current_err_msg';
                break;
            case 'x':
                ...
                case 'err_no':
                case 'err_msg':
                    $action[] = 'MLog::$current_instance->current_'.$param;
                    break;
                case 'log_id':
                    $action[] = "MLog::genLogID()";
                    break;
                case 'app':
                    $action[] = "MLog::getLastLogger()";
                    break;
                case 'function_param':
                    $action[] = 'MLog::flattenArgs(MLog::$current_instance->current_function_param)';
                    break;
                case 'argv':
                    $action[] = '(isset($GLOBALS["argv"])? MLog::flattenArgs($GLOBALS["argv"]) : \'\')';
                    break;
                case 'pid':
                    $action[] = 'posix_getpid()';
                    break;
                case 'encoded_str_array':
                    $action[] = 'MLog::$current_instance->get_str_args_std()';
                    break;
                case 'cookie':
                    $action[] = "(isset(\$_SERVER['HTTP_COOKIE'])? \$_SERVER['HTTP_COOKIE'] : '')";
                    break;
     ...
     * @param MLog_Writer $writer
     */
    private static function registerWriter(MLog_Writer $writer) {
    ...
    public static function useLogerStdOut() {
        ...
        self::registerWriter(new MLog_Writer_Std());
        $used = true; 
    }
}
